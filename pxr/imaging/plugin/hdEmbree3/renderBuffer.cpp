...
HdEmbree3RenderBuffer::HdEmbree3RenderBuffer(SdfPath const& id)
    : HdRenderBuffer(id)
    , _width(0)
    , _height(0)
...
HdEmbree3RenderBuffer::~HdEmbree3RenderBuffer()
{
}

/*virtual*/
void
HdEmbree3RenderBuffer::Sync(HdSceneDelegate *sceneDelegate,
                           HdRenderParam *renderParam,
                           HdDirtyBits *dirtyBits)
{
    if (*dirtyBits & DirtyDescription) {
        // Embree3 has the background thread write directly into render buffers,
        // so we need to stop the render thread before reallocating them.
        static_cast<HdEmbree3RenderParam*>(renderParam)->AcquireSceneForEdit();
    }
...
/*virtual*/
void
HdEmbree3RenderBuffer::Finalize(HdRenderParam *renderParam)
{
    // Embree3 has the background thread write directly into render buffers,
...
/*virtual*/
void
HdEmbree3RenderBuffer::_Deallocate()
{
...
/*static*/
size_t
HdEmbree3RenderBuffer::_GetBufferSize(GfVec2i const &dims, HdFormat format)
{
...
/*static*/
HdFormat
HdEmbree3RenderBuffer::_GetSampleFormat(HdFormat format)
{
...
/*virtual*/
bool
HdEmbree3RenderBuffer::Allocate(GfVec3i const& dimensions,
                               HdFormat format,
                               bool multiSampled)
{
    _Deallocate();
...
void
HdEmbree3RenderBuffer::Clear(size_t numComponents, float const* value)
{
    size_t formatSize = HdDataSizeOfFormat(_format);
...
void
HdEmbree3RenderBuffer::Clear(size_t numComponents, int const* value)
{
    size_t formatSize = HdDataSizeOfFormat(_format);
    for (size_t i = 0; i < _width*_height; ++i) {
        uint8_t *dst = &_buffer[i*formatSize];
