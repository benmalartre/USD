...
//
#ifndef PXR_IMAGING_PLUGIN_HD_EMBREE3_MESH_H
#define PXR_IMAGING_PLUGIN_HD_EMBREE3_MESH_H
...
/// \class HdEmbree3Mesh
///
/// An HdEmbree3 representation of a subdivision surface or poly-mesh object.
...
/// is deferred to HdEmbree3RenderDelegate::CommitResources(), which runs after
/// all prims have been updated. After running Sync() for each prim and
/// HdEmbree3RenderDelegate::CommitResources(), the scene should be ready for
...
/// each other.
///
class HdEmbree3Mesh final : public HdMesh {
public:
    HF_MALLOC_TAG_NEW("new HdEmbree3Mesh");

    /// HdEmbree3Mesh constructor.
    ///   \param id The scene-graph path to this mesh.
    ///   \param instancerId If specified, the HdEmbree3Instancer at this id uses
    ///                      this mesh as a prototype.
    HdEmbree3Mesh(SdfPath const& id,
                 SdfPath const& instancerId = SdfPath());

    /// HdEmbree3Mesh destructor.
    /// (Note: Embree3 resources are released in Finalize()).
    virtual ~HdEmbree3Mesh() = default;
    ...
    ///   \param renderParam An HdEmbree3RenderParam object containing top-level
    ///                      embree state.
...
private:
    ...
    // carefully.
    HdEmbree3PrototypeContext* _GetPrototypeContext();
    HdEmbree3InstanceContext* _GetInstanceContext(RTCScene scene, size_t i);
    ...
    void _CreateEmbree3SubdivMesh(RTCScene scene);
    // Utility function to call rtcNewTriangleMesh and populate topology.
    void _CreateEmbree3TriangleMesh(RTCScene scene);

    // An embree intersection filter callback, for doing backface culling.
    static void _Embree3CullFaces(void *userData, RTCRay &ray);

private:
    // Every HdEmbree3Mesh is treated as instanced; if there's no instancer,
    ...
    HdEmbree3Mesh &operator =(const HdEmbree3Mesh&) = delete;
};

PXR_NAMESPACE_CLOSE_SCOPE

#endif // PXR_IMAGING_PLUGIN_HD_EMBREE3_MESH_H
