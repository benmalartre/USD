...
HdEmbree3Instancer::HdEmbree3Instancer(HdSceneDelegate* delegate,
                                     SdfPath const& id,
...
HdEmbree3Instancer::~HdEmbree3Instancer()
{
...
void
HdEmbree3Instancer::_SyncPrimvars()
{
    HD_TRACE_FUNCTION();
...
VtMatrix4dArray
HdEmbree3Instancer::ComputeInstanceTransforms(SdfPath const &prototypeId)
{
    HD_TRACE_FUNCTION();
    HF_MALLOC_TAG_FUNCTION();
    ...
    if (_primvarMap.count(HdInstancerTokens->rotate) > 0) {
        HdEmbree3BufferSampler sampler(*_primvarMap[HdInstancerTokens->rotate]);
        for (size_t i = 0; i < instanceIndices.size(); ++i) {
            GfVec4f quat;
    ...
    if (_primvarMap.count(HdInstancerTokens->scale) > 0) {
        HdEmbree3BufferSampler sampler(*_primvarMap[HdInstancerTokens->scale]);
        for (size_t i = 0; i < instanceIndices.size(); ++i) {
            GfVec3f scale;
    ...
    VtMatrix4dArray parentTransforms =
        static_cast<HdEmbree3Instancer*>(parentInstancer)->
