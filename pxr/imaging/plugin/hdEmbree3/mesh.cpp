...
#include "pxr/imaging/plugin/hdEmbree3/context.h"
...
HdEmbree3Mesh::HdEmbree3Mesh(SdfPath const& id,
...
void
HdEmbree3Mesh::Finalize(HdRenderParam *renderParam)
{
    RTCScene scene = static_cast<HdEmbree3RenderParam*>(renderParam)
        ->AcquireSceneForEdit();
...
HdDirtyBits
HdEmbree3Mesh::GetInitialDirtyBitsMask() const
{
...
HdDirtyBits
HdEmbree3Mesh::_PropagateDirtyBits(HdDirtyBits bits) const
{
    return bits;
}

void
HdEmbree3Mesh::_InitRepr(TfToken const &reprToken,
...
void
HdEmbree3Mesh::Sync(HdSceneDelegate *sceneDelegate,
                   ...
                   TfToken const   &reprToken)
{
    ...
    // For now, HdEmbree3Mesh only respects the first desc; this should be fixed.
    _MeshReprConfig::DescArray descs = _GetReprDesc(reprToken);
    const HdMeshReprDesc &desc = descs[0];

    // Pull top-level embree state out of the render param.
    HdEmbree3RenderParam *embreeRenderParam =
        static_cast<HdEmbree3RenderParam*>(renderParam);
    RTCScene scene = embreeRenderParam->AcquireSceneForEdit();
    RTCDevice device = embreeRenderParam->GetEmbree3Device();
...
HdEmbree3Mesh::_Embree3CullFaces(void *userData, RTCRayHit &ray)
{
    ...
    // Only HdEmbree3Mesh gets HdEmbree3Mesh::_Embree3CullFaces bound
    // as an intersection filter. The filter is bound to the prototype,
    // whose context's rprim always points back to the original HdEmbree3Mesh.
    HdEmbree3PrototypeContext *ctx =
        static_cast<HdEmbree3PrototypeContext*>(userData);
...
void
HdEmbree3Mesh::_CreateEmbree3SubdivMesh(RTCScene scene)
{
...
void
HdEmbree3Mesh::_CreateEmbree3TriangleMesh(RTCScene scene)
{
...
void
HdEmbree3Mesh::_UpdatePrimvarSources(HdSceneDelegate* sceneDelegate,
                                    HdDirtyBits dirtyBits)
{
    HD_TRACE_FUNCTION();
...
TfTokenVector
HdEmbree3Mesh::_UpdateComputedPrimvarSources(HdSceneDelegate* sceneDelegate,
                                            HdDirtyBits dirtyBits)
{
    HD_TRACE_FUNCTION();
...
void
HdEmbree3Mesh::_CreatePrimvarSampler(TfToken const& name, VtValue const& data,
                                    HdInterpolation interpolation,
                                    bool refined)
{
    // Delete the old sampler, if it exists.
    HdEmbree3PrototypeContext *ctx = _GetPrototypeContext();
    ...
    // geometry mode.
    HdEmbree3PrimvarSampler *sampler = nullptr;
    switch(interpolation) {
        case HdInterpolationConstant:
            sampler = new HdEmbree3ConstantSampler(name, data);
            break;
        case HdInterpolationUniform:
            if (refined) {
                sampler = new HdEmbree3UniformSampler(name, data);
            } else {
                sampler = new HdEmbree3UniformSampler(name, data,
                    _trianglePrimitiveParams);
            }
        ...
        case HdInterpolationFaceVarying:
            if (refined) {
                // XXX: Fixme! HdEmbree3 doesn't currently support face-varying
                // primvars on subdivision meshes.
                TF_WARN("HdEmbree3Mesh doesn't support face-varying primvars"
                        " on refined meshes.");
            } else {
                HdMeshUtil meshUtil(&_topology, GetId());
                sampler = new HdEmbree3TriangleFaceVaryingSampler(name, data,
                    meshUtil);
            }
            break;
        default:
...
void
HdEmbree3Mesh::_PopulateRtMesh(HdSceneDelegate* sceneDelegate,
                              RTCScene         scene,
                              RTCDevice        device,
                              HdDirtyBits*     dirtyBits,
                              HdMeshReprDesc const &desc)
{
    ...
    if (HdChangeTracker::IsTopologyDirty(*dirtyBits, id) ||
        ...
        if (doRefine) {
            _CreateEmbree3SubdivMesh(_rtcMeshScene);
        } else {
            _CreateEmbree3TriangleMesh(_rtcMeshScene);
        }
        _refined = doRefine;
    ...
    if (newMesh || HdChangeTracker::IsDisplayStyleDirty(*dirtyBits, id)) {
        if (doRefine) {
            // Pass the target number of uniform refinements to Embree3.
            // Embree3 refinement is specified as the number of quads to generate
            ...
            // XXX: As of Embree3 2.9.0, rendering with tessellation level 1
    ...
    // HdEmbree3 to tell whether transforms will be dirty, so this code
    ...
    if (!GetInstancerId().IsEmpty()) {
        ...
        VtMatrix4dArray transforms =
            static_cast<HdEmbree3Instancer*>(instancer)->
        ...
        for(size_t i = oldSize; i < newSize; ++i) {
            ...
            HdEmbree3InstanceContext *ctx = new HdEmbree3InstanceContext;
...
HdEmbree3PrototypeContext*
HdEmbree3Mesh::_GetPrototypeContext()
{
    return static_cast<HdEmbree3PrototypeContext*>(
        rtcGetUserData(_rtcMeshScene, _rtcMeshId));
}

HdEmbree3InstanceContext*
HdEmbree3Mesh::_GetInstanceContext(RTCScene scene, size_t i)
{
    return static_cast<HdEmbree3InstanceContext*>(
