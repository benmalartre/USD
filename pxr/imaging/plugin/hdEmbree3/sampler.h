...
//
#ifndef PXR_IMAGING_PLUGIN_HD_EMBREE3_SAMPLER_H
#define PXR_IMAGING_PLUGIN_HD_EMBREE3_SAMPLER_H
...
// Define template specializations of HdEmbree3TypeHelper methods for
// all our supported types...
#define TYPE_HELPER(T,type)\
template<> inline HdTupleType \
HdEmbree3TypeHelper::GetTupleType<T>() { return HdTupleType{type, 1}; }
...
public:
    ...
    HdEmbree3BufferSampler(HdVtBufferSource const& buffer)
        : _buffer(buffer) {}
    ...
    template<typename T> bool Sample(int index, T* value) const {
        return Sample(index, static_cast<void*>(value),
            HdEmbree3TypeHelper::GetTupleType<T>());
    }
...
public:
    /// Default constructor.
    HdEmbree3PrimvarSampler() = default;
    /// Default destructor.
    virtual ~HdEmbree3PrimvarSampler() = default;
...
#endif // PXR_IMAGING_PLUGIN_HD_EMBREE3_SAMPLER_H
