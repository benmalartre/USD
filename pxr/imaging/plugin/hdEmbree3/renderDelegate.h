...
//
#ifndef PXR_IMAGING_PLUGIN_HD_EMBREE3_RENDER_DELEGATE_H
#define PXR_IMAGING_PLUGIN_HD_EMBREE3_RENDER_DELEGATE_H
...
class HdEmbree3RenderParam;

#define HDEMBREE3_RENDER_SETTINGS_TOKENS \
...
TF_DECLARE_PUBLIC_TOKENS(HdEmbree3RenderSettingsTokens, HDEMBREE3_RENDER_SETTINGS_TOKENS);

///
/// \class HdEmbree3RenderDelegate
///
...
///
/// HdEmbree3 Rprims create embree geometry objects in the render delegate's
/// top-level embree scene; and HdEmbree3's render pass draws by casting rays
...
/// CommitResources(). This can be used to commit GPU buffers or, in HdEmbree3's
/// case, to do a final build of the BVH.
///
class HdEmbree3RenderDelegate final : public HdRenderDelegate {
public:
    ...
    /// HdEmbree3RenderPass, which draw using embree's raycasting API.
    ...
    /// embree-specialized geometry containers like HdEmbree3Mesh which map
    ...
    /// Embree3 doesn't currently use materials but raytraced backends generally
    /// specify "full".
...
private:
    ...
    HdEmbree3RenderDelegate(const HdEmbree3RenderDelegate &)             = delete;
    HdEmbree3RenderDelegate &operator =(const HdEmbree3RenderDelegate &) = delete;

    // Embree3 initialization routine.
    void _Initialize();
    ...
    // A shared HdEmbree3RenderParam object that stores top-level embree state;
    // passed to prims during Sync().
    std::shared_ptr<HdEmbree3RenderParam> _renderParam;
...
#endif // PXR_IMAGING_PLUGIN_HD_EMBREE3_RENDER_DELEGATE_H
