...
//
#include "pxr/imaging/plugin/hdEmbree3/renderer.h"

#include "pxr/imaging/plugin/hdEmbree3/renderBuffer.h"
...
HdEmbree3Renderer::HdEmbree3Renderer()
    : _aovBindings()
    , _aovNames()
...
void
HdEmbree3Renderer::SetScene(RTCScene scene)
{
    _scene = scene;
}

void
HdEmbree3Renderer::SetSamplesToConvergence(int samplesToConvergence)
{
    _samplesToConvergence = samplesToConvergence;
}

void
HdEmbree3Renderer::SetAmbientOcclusionSamples(int ambientOcclusionSamples)
{
    _ambientOcclusionSamples = ambientOcclusionSamples;
}

void
HdEmbree3Renderer::SetEnableSceneColors(bool enableSceneColors)
{
    _enableSceneColors = enableSceneColors;
}

void
HdEmbree3Renderer::SetViewport(unsigned int width, unsigned int height)
{
    _width = width;
    _height = height;
...
void
HdEmbree3Renderer::SetCamera(const GfMatrix4d& viewMatrix,
...
void
HdEmbree3Renderer::SetAovBindings(
...
bool
HdEmbree3Renderer::_ValidateAovBindings()
{
...
/* static */
GfVec4f
HdEmbree3Renderer::_GetClearColor(VtValue const& clearValue)
{
...
HdEmbree3Renderer::Clear()
{
    ...
    for (size_t i = 0; i < _aovBindings.size(); ++i) {
        ...
        HdEmbree3RenderBuffer *rb = 
            static_cast<HdEmbree3RenderBuffer*>(_aovBindings[i].renderBuffer);
...
void
HdEmbree3Renderer::MarkAovBuffersUnconverged()
{
    for (size_t i = 0; i < _aovBindings.size(); ++i) {
        HdEmbree3RenderBuffer *rb =
            static_cast<HdEmbree3RenderBuffer*>(_aovBindings[i].renderBuffer);
...
int
HdEmbree3Renderer::GetCompletedSamples() const
{
    return _completedSamples.load();
}

void
HdEmbree3Renderer::Render(HdRenderThread *renderThread)
{
    ...
    if (!_ValidateAovBindings()) {
        ...
        for (size_t i = 0; i < _aovBindings.size(); ++i) {
            HdEmbree3RenderBuffer *rb = static_cast<HdEmbree3RenderBuffer*>(
    ...
    for (size_t i = 0; i < _aovBindings.size(); ++i) {
        static_cast<HdEmbree3RenderBuffer*>(
    ...
    for (int i = 0; i < _samplesToConvergence; ++i) {
        ...
        unsigned int tileSize = HdEmbree3Config::GetInstance().tileSize;
        ...
        WorkParallelForN(numTilesX*numTilesY,
            std::bind(&HdEmbree3Renderer::_RenderTiles, this,
...
void
HdEmbree3Renderer::_RenderTiles(HdRenderThread *renderThread,
                               size_t tileStart, size_t tileEnd)
{
    unsigned int tileSize =
        HdEmbree3Config::GetInstance().tileSize;
    const unsigned int numTilesX = (_width + tileSize-1) / tileSize;
    ...
    for (unsigned int tile = tileStart; tile < tileEnd; ++tile) {
        ...
        for (unsigned int y = y0; y < y1; ++y) {
            for (unsigned int x = x0; x < x1; ++x) {
                ...
                if (HdEmbree3Config::GetInstance().jitterCamera) {
...
void
HdEmbree3Renderer::_TraceRay(unsigned int x, unsigned int y,
                            ...
                            std::default_random_engine &random)
{
    ...
    for (size_t i = 0; i < _aovBindings.size(); ++i) {
        HdEmbree3RenderBuffer *renderBuffer =
            static_cast<HdEmbree3RenderBuffer*>(_aovBindings[i].renderBuffer);
...
float
HdEmbree3Renderer::_ComputeAmbientOcclusion(GfVec3f const& position,
